...
void SYCLModuleNode::Init() {
  ...
  std::string cmd = sycl_compiler +" "+sycl_flags +" "+ filepath+" -o "+sharedlibpath;
  ...
  for (auto& kv : parsed_kernels_) {
    ...
    myfile << kernel_code;
    myfile.close();
    std::string cmd = "$DPCPP_HOME/build-cuda/bin/clang++ -std=c++17 -O3 -fsycl -fsycl-targets=nvptx64-nvidia-cuda -fPIC -shared " + filepath + " -o " + sharedlibpath;
    system(cmd.c_str());
    VLOG(0) << cmd;
  }*/
}
